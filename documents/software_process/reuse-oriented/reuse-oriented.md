...
Sommerville's own case for reuse-oriented development rests on two practical points that carry extra weight for infrastructure like Mental health care's: [S5]
...
More effective use of development resources, since the team's time goes toward configuration and integration testing rather than reinventing solved engineering problems, freeing effort for the parts of Mental health care that actually are unique to a mental health clinic (like the clinical record-keeping and safety-flag logic covered under plan-driven).

Mental health care's infrastructure requirements — encryption, authentication hardware, server clustering, offline sync — are not unique to Mental health care or to healthcare; they're general-purpose engineering problems that the wider software industry has already solved and hardened through years of real-world use, which is exactly the condition under which the book favors reuse over custom-building.
...
| SR-NF08 | Linux-based data centre infrastructure | Existing operating system and hosting infrastructure, not built by the Mental health care team |

## Reuse-Oriented — How it works :
Integrate an existing smart card authentication module and configure it against staff employee IDs (SR-NF02). Integrate existing TLS 1.3 and AES-256 encryption libraries into the data pipeline (SR-NF03). Configure a standard server cluster with load balancing and heartbeat-based failover (SR-NF01) on the health authority's existing Linux data centre (SR-NF08). Integrate existing database replication/offline-sync technology for clinics without reliable connectivity (SR-F07). Each component is validated through integration testing — confirming the reused component works correctly with Mental health care's core system — rather than unit testing new logic, since the logic itself already exists and is proven.
