...
Sommerville's own reasoning for reuse-oriented development centers on avoiding unnecessary, risky, and costly custom engineering when a proven solution already exists. Two points map directly onto Mental health care's infrastructure needs:
...
* **Faster time to a working system:** because the components already exist and are proven, the team spends its effort on *configuration and integration* — connecting Mental health care's core application to a smart card reader, a TLS library, a Linux server cluster — rather than *inventing* those pieces.

Both reasons matter more, not less, for safety-critical infrastructure like Mental health care's: a custom-built encryption or failover system would need extensive independent security validation before anyone could trust it, whereas TLS 1.3, AES-256, and standard clustering software already carry that validation from widespread real-world use.
...
| **SR-NF08** | Linux-based data centre infrastructure | Existing operating system and data-centre hardware/hosting, not built by the Mental health care team |
